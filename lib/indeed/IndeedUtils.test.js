...
describe('IndeedUtils', () => {
  ...
  describe('static createSearchJobsUrl', () => {
    test('it should create the correct url', () => {
      ...
      const result2 = IndeedUtils.createSearchJobsUrl(
        { searchTerm: 'nodejs', location: 'Montréal, QC', limit: 25 },
      ...
      expect(result2).toBe('https://ca.indeed.com/jobs?q=nodejs&l=Montr%C3%A9al%2C+QC&limit=25');
  ...
  describe('static createApplyUrl', () => {
    test('it should create the correct url', () => {
      const result1 = IndeedUtils.createApplyUrl({ indeedId: 'id1234' });
      const result2 = IndeedUtils.createApplyUrl({ indeedId: 'id1234' }, { host: 'https://ca.indeed.com' });

      expect(result1).toBe('https://www.indeed.com/rc/clk?jk=id1234&from=vj');
      expect(result2).toBe('https://ca.indeed.com/rc/clk?jk=id1234&from=vj');
    });
  });
});
