...
class IndeedScraper extends BaseScraper {
  static defaultHost = 'https://www.indeed.com/';

  static createIndeedUrl(urlPath, { host } = {}) {
    return new URL(urlPath, host || IndeedScraper.defaultHost).toString();
  }

  static createSearchJobsUrl({ searchTerm, location, radius }, { host } = {}) {
    const url = new URL('/jobs', host || IndeedScraper.defaultHost);
